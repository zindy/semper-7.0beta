/*
    ...
    Modify: Christian Dwyer     15/Aug/2005 - Explicit casting for ANSI C functions
*/
...
MemorySize data_size;
{
    ...
    if ( data_size > 0 )
    {
	data_address = (MemoryAddress) malloc ( (size_t) data_size );
    }
...
MemorySize    data_size;
{
    ...
    {
	new_data_address = (MemoryAddress) realloc ( data_address, (size_t) data_size );
    }
...
DeviceSize    data_count;
{
    if ( data_count > 0 )
    {
	memcpy ( (void *) destination_buffer, (void *) source_buffer, (size_t) data_count );
    }
}
...
DeviceSize    data_count;
{
    if ( data_count > 0 )
    {
	memset ( (void *) data_buffer, 0, (size_t) data_count );
    }
}
...
DeviceSize  *file_size;
{
    ...
    size = (DeviceSize) lseek ( handle, (off_t) 0, L_XTND );
...
DeviceSize new_file_size;
{
    ...
    old_file_size = (DeviceSize) lseek ( file_handle, (off_t) 0, L_XTND );

    if ( old_file_size == -1 ) return ( TRUE );

    if ( new_file_size < old_file_size )
    {
	if ( ftruncate ( file_handle, (off_t) new_file_size ) == -1 ) return ( TRUE );
    }
	...
	{
	    ftruncate ( file_handle, (off_t) old_file_size );
...
DeviceSize    data_count;
{
	...
	if ( read ( file_handle, data_buffer, (size_t) data_count ) != (ssize_t) data_count 
) return ( TRUE );
    }
...
DeviceSize    data_count;
{
    ...
    if ( data_count > 0 )
    {
	if ( lseek ( file_handle, (off_t) file_offset, L_SET ) == -1 ) return ( TRUE );

	if ( write ( file_handle, data_buffer, (size_t) data_count ) != (ssize_t) data_count 
) return ( TRUE );
    }
...
DeviceSize    data_count;
{
	...
	{
	    if ( write ( file_handle, zero_buffer, (size_t) zero_size ) != (ssize_t) 
zero_size ) return ( TRUE );
	...
	if ( write ( file_handle, zero_buffer, (size_t) zero_size ) != (ssize_t) zero_size ) 
return ( TRUE );
    }
...
SegmentIndex segment_index;
{
	...
	{
	    segment_address = (SegmentAddress) mmap ( (void *) NULL, (size_t) segment_size,
				     PROT_READ | PROT_WRITE, MAP_SHARED,
				     file_handle, (off_t) file_offset );
	}
...
SegmentIndex segment_index;
{
	...
	if ( munmap ( (void *) segment_address, (size_t) segment_size ) != 0 ) return ( TRUE );
...
#ifdef CACHE_MMAP
	    if ( msync ( (void *) segment_address, (size_t) segment_size, MS_ASYNC ) != 0 ) return ( TRUE );
#endif
#ifdef CACHE_MALLOC
	    file_handle = device_file_handle [ device_number ];
...
F_integer     *DISCF;
{
    ...
    if ( block_number < 1 || data_count < 0 ) 
	{
	printf("\n TT1 ");
	return ( TRUE );
	}
    ...
    if ( (device_offset + device_byte_count) > device_size ) 
        {
        printf("\n TT4 %d, %d, %d \n",device_offset,device_byte_count,device_size);
	printf("TT4_2 %d \n",device_offset+device_byte_count-device_size);
	printf("TT4_3 %d \n",device_offset+device_byte_count);
        return ( TRUE );
        }
	...
	else
	{
	    ...
	    if ( disc_byte_increment != disc_byte_count ) 
                {
                        printf("\n Ug 2 ");
			return ( TRUE );
		}
	...
	if ( memory_data_bytes == disc_data_bytes )
	{
	    if ( write_file ( file_handle, device_offset, data_buffer, device_byte_count ) ) 
                {
                        printf("\n Ug 7 ");
                        return ( TRUE );
		}
	...
	else
	{
	    ...
	    if ( write_file ( file_handle, device_offset, io_buffer, device_byte_count ) ) 
		{
			printf("\n Ug 10");
			return ( TRUE );
		}
