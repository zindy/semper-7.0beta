...
c----------------------------------------------------------------------
c
c	LOGICAL FUNCTION WINTEX ( DEVICE,WID,FGC,BGC,STYLE,XPOS,YPOS,
c                                 STRING, LENGTH )
c       ---------------------------------------------------------------
c
c	PARAMETERS:
...
c----------------------------------------------------------------------
c
      logical function wintex ( device, wid, fgc, bgc, style, xpos,
     +                          ypos, string, length )
c     ==========================================================
c
...
c     CALLED FUNCTIONS:
c
c     Writes text on host device window
      logical whotex
c
c     Write text on framestore device window
      logical wfstex
c
      if ( device .eq. HOST ) then
         status = whotex ( device, wid, fgc, bgc, style, xpos,
     +                     ypos, string, length )
      else
         status = wfstex ( device, wid, fgc, bgc, style, xpos,
     +                     ypos, string, length )
      endif
c
      wintex = status
