...
c----------------------------------------------------------------------
c
c	LOGICAL FUNCTION WINBOX ( DEVICE, WID, WIDTH, FGC, BGC,
...
c                              coordinates
c
c	Draws a box to the requested window, in the appropriate
c       colours and style.  If requested, a string will be written into
c       the top edge of the box.
c
c	Function returns FALSE if sucessful, otherwise TRUE.
c
c----------------------------------------------------------------------
c
      logical function winbox ( device, wid, width, fgc, bgc,
...
c     CALLED FUNCTIONS:
c
c     Writes box into host device window
      logical whobox
c
c     Write box into framestore device window
      logical wfsbox
c
      if ( device .eq. HOST ) then
         status = whobox ( device, wid, width, fgc, bgc,
     +          string, length, style, pos, xpos, ypos, xsize, ysize )
      else
         status = wfsbox ( device, wid, width, fgc, bgc,
     +          string, length, style, pos, xpos, ypos, xsize, ysize )
      endif
c
      winbox = status
