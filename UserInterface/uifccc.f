c----------------------------------------------------------------------
c
c       LOGICAL FUNCTION UIFCCC ( EID, CYCLES )
c       ---------------------------------------
c
c       PARAMETERS:
c
c       integer eid : INPUT - the identifier of the check highlighing
c                             cell whose highlight state is to be cycled
c
c       integer cycles : INPUT - the number of cycles to be performed.
c
c       Cycles the given cell, which must have check highlighting set,
c       through its highlighting states by a given number of cycles.
c       The highlighting states of the cell are CROSS, NONE, TICKED.
...
c----------------------------------------------------------------------
c
      logical function uifccc ( eid, cycles )
c     =======================================
c
      integer eid, cycles
c
...
c
                  do 10 i = 1, cycles
                     if      ( celcyc(eid) .eq. CROSS ) then
                        celcyc(eid) = NONE
                     else if ( celcyc(eid) .eq. NONE ) then
                        celcyc(eid) = TICKED
