...
%INCLUDE '../source/COMMON'
       INTEGER MAXRL,MAXLIN,MINRAD,MAXLRL
       PARAMETER(MAXRL=512,MAXLIN=60,MINRAD=5)
* NB the size of output buffer opb depends on minimum radius
...
*  arrays
	common /noddy/RB7LHS,RB7,RB7RHS,RB8LHS,RB8,RB8RHS
       ...
       EQUIVALENCE(RB8(1791),SIDE),(RB8(890),HIS)
* small variables
       equivalence (RB8(M+6),R),(RB8(M+7),TOT2),(RB8(M+8),JUMP)
       EQUIVALENCE (RB8(M+11),NOHCLS)
       border3=0
...
4          SIDE(K)=RAD
      ELSEif(Ocir)then
        ...
        do 10 i=-rad,-1
           do 11 j = k,-rad,-1 
              if(nint(sqrt(i**2+j**2)).le.rad)then
                 side(i)=abs(j)
                 side(-i)=abs(j)
              else
                 go to 112
...
*            GR=IB(RP(LP,RI)+(IC+SIDE(K))+1)
            ...
            if(.not.lmean)then 
               ...
               sum0=sum0+int4(gr)-int4(gl)
            endif
302       CONTINUE
          if(lmean)then
              jj=nint(float(sum0)/float(int4(tot0)))
          else
...
* see above
          ...
          if(.not.lmean)then
             ...
             sum1=sum1+int4(gb)-int4(gt)
          endif
303     CONTINUE
       if(lmean)then
         sum0=sum1
         opb(bord-border)=nint(float(sum0)/float(int4(tot0)))
       else
