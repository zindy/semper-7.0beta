...
C
C      write(6,*)'SETPPR ' !LDM
      IF (SETPPR()) THEN
         RECORD = 'Call to SETPPR'
         GOTO 180
      ENDIF
C
C Check for valid password
C
C      write(6,*)'Password ' !LDM
      IF (SEMCPW(SERNUM(15:34))) GOTO 230
C
C Initialise framestore routines
C
C      write(6,*)'FSIN61 ' !LDM
...
C      write(6,*)'Help ' !LDM
      ...
      IF (IND .NE. 0) THEN
         ...
         GOTO 200
      ENDIF
C      write(6,*)'Help 2' !LDM
C Also use the more conventional --help, -h
      IND = INDEX(LINE,'/h')
      IF (IND .EQ. 0) IND = INDEX(LINE,'-h')
      IF (IND .NE. 0) THEN
         CALL SEMUSA
         GOTO 200
      ENDIF
      IND = INDEX(LINE,'/-help')
      IF (IND .EQ. 0) IND = INDEX(LINE,'--help')
      IF (IND .NE. 0) THEN
         CALL SEMUSA
         GOTO 200
      ENDIF
...
C
C      write(6,*)'Opening run' !LDM
      IU = RUNFLE
      IND = INDEX(LINE,'/norun')
      IF (IND .EQ. 0) IND = INDEX(LINE,'-norun')
      IF (IND .EQ. 0) THEN
C         write(6,*)'Semin2 line ' !LMD
         IF (SEMIN2(LINE,UCLINE,FILENM,'r','.run',FOUND)) GOTO 270
C         write(6,*)'run OK' !LDM
      ELSE
         FOUND = .FALSE.
         LINE(IND:IND+5) = '      '
         UCLINE(IND:IND+5) = '      '
      ENDIF
C      write(6,*)'Found for run ',found !LDM
      IF (FOUND) THEN
...
C
  ...
  160 CONTINUE
C
C      IF (SEMCON('Semper 6 Plus: '//
C     + 'Copyright (C) 1987-1996 Synoptics Ltd.')) GOTO 200
C      IF (SEMCON('Portable Semper 7')) goto 200
      LINE(1:4)='    '
      if(SEMCON(LINE(1:4)))goto 200
C
      ISOUT1 = SempBuild(2,LINE)
      LINE(1:ISOUT1)=LINE(1:ISOUT1)
      ISOUT1=ISOUT1+1
      LINE(ISOUT1:ISOUT1)=' '
      ISOUT1=ISOUT1+1
      ...
      IF(SEMCON(LINE(1:ISOUT1))) goto 200
      STRIKE = '-----------------------------------------------------'
      IF (SEMCON(STRIKE)) GOTO 200
C
C     IF (SEMCON(SERNUM)) GOTO 200
C
...
C
  ...
  220 CONTINUE
C     Use Pause, since something horrible has probably happened
C     PAUSE
C      CALL WAITS(25.0)
  230 CONTINUE
C
