...
C
      ...
      IF (SMGI9 .GT. SHORTF(MAXLYR)) THEN
        ERROR = EBASRS + 2
        GOTO 120
...
C
      HITS = 0
      DO 40 I = 1, (NCLAS - 1)
        IX0 = LONGF((I - 1) * SIZCLA + 1)
        DO 30 J = (I + 1), NCLAS
          IX1 = LONGF((J - 1) * SIZCLA + 1)
...
C
      BASE = SMGI2
      BASE4 = LONGF(NL + 2) * LONGF(NL) + LNGONE
      INCR = LONGF(NL)
      STEPS4 = LONGF(NCOLS(LP1))
      DO 110 I = 1, NY
        IX0 = LNGONE
        DO 50 J = SMGI3, SMGI6
...
C
   70       IX1 = IX1 + LONGF(SIZCLA)
   80     CONTINUE
   90     IX0 = IX0 + LONGF(STEP)
  100   CONTINUE
