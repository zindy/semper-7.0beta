C Semper 6 processing module LABEL
C
      SUBROUTINE LABEL
C
C Generates a labeled version of the source picture where each
C connected region is uniquely numbered
C
      LOGICAL OPT,SEMOPN,SEMROW,LABEL2,LABEL4,SEMRNG,SEMMED
      INTEGER IVALPN !,IPACK
C
      INTEGER I,J,K,N,KN,KO,KOF,KOP,KNF,KNP,KOB,FG,FG1,FG2,MEDIUM
      INTEGER NEWID,OLDID,CURID,NEXID,IMIN,IMAX,OLDLNK,NEWLNK,J1,J2
      INTEGER NCOL,NROW,NREG,NPIC,NCHD,NSTART,NEND,NOVER(0:1)
      INTEGER LASCHD,MAXCHD,MAXREG,KOLD,NOLD,KNEW,NNEW,LPT
      LOGICAL LFG,LBG
C
      INCLUDE 'COMMON'
C
...
C
      NPIC=IVALPN(-601)
      LP2=LP1
      IF (SEMOPN(2,NPIC,NCOL,NROW,1,NCLIMA,NFMINT,LP2)) GOTO 170
C
C Fetch medium associated with output picture
C
      IF (SEMMED(DEVN(LP2),MEDIUM)) GOTO 170
C
C If output picture is display picture, open a temporary integer form
C picture to guarantee integrity of intermediate results
C
      IF (MEDIUM.EQ.MEDDS) THEN
         LPT=0
         IF (SEMOPN(3,0,NCOL,NROW,1,NCLIMA,NFMINT,LPT)) GOTO 170
C
C Otherwise, use output picture for storing intermediate results
C
      ELSE
         LPT=LP2
      ENDIF
...
C
         IF (LABEL2(LP1,J,NCHORD-NOLD,KNEW,NNEW)) GOTO 170
C
...
C
            IF (SEMROW(2,IB1,NFMINT,J,1,LPT)) GOTO 170
         ENDIF
C
...
C
  ...
  140 CONTINUE
C
C Set up picture range values (lowest value may be 0 or 1)
C
      IMIN=1
      IMAX=NREG
C
...
C
         IF (SEMROW(1,IB1,NFMINT,J,1,LPT)) GOTO 170
C
...
C
         IF (SEMROW(2,IB1,NFMINT,J,1,LP2)) GOTO 170
  160 CONTINUE
C
...
C
      END
C Semper 6 subsidiary module LABEL2
C
      LOGICAL FUNCTION LABEL2(LPN,ROW,NMAX,KNEW,NNEW)
C
      INTEGER LPN,ROW,NMAX,KNEW,NNEW
C
...
C
      IF (SEMROW(1,PIXEL,NFMINT,ROW,1,LPN)) GOTO 60
C
